...
public class StoreProcedureOutParameter {
    ...
    public static void main(String[] args) {
        ...
        String createSP = "CREATE OR REPLACE PROCEDURE get_employee_by_id( "
                + " p_id IN EMPLOYEE3.ID%TYPE, "
                + " o_name OUT EMPLOYEE3.NAME%TYPE, "
                + " o_salary OUT EMPLOYEE3.SALARY%TYPE, "
                + " o_date OUT EMPLOYEE3.CREATED_DATE%TYPE) "
                + " AS "
                + " BEGIN "
                + "     SELECT NAME, SALARY, CREATED_DATE INTO o_name, o_salary, o_date from EMPLOYEE3 WHERE ID = p_id; "
                + " END;";
